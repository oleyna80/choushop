...
| 2026-06-17 | wb-catalog-ux-filter | Lite tier | Codex Control Tower delegated WB. Catalog filter UX polish — component-level UI change, no DB/auth/payment/API. 3 files. | reviewer + verifier (planned) | critic: SKIPPED — Codex pre-scoped delegation; component-level UI, no new domain, no DB/auth triggers | In progress |
...
| 2026-06-16 | wb-candy-cloud-public-frontend | Full tier | Implement the Candy Cloud public frontend from `codex_mystery_box_design_brief.md` and `candy_cloud_site_sketch.png` using Codex-only SDLC. Public frontend, mock data, routes, cart/request flow, visual system, and SDLC logs are in scope; admin/auth/Prisma/package/backend/payment and Claude Code delegation are out of scope. | Frontend Analyst (read-only Codex subagent) | critic: SUPPLEMENT — route/backend risk handled by mock product module, Vinted request flow, old-route redirects, browser smoke, and no admin/backend/package/schema edits | READY |
| 2026-06-17 | wb-cc-handoff-review-smoke | Standard tier | Validate Claude Code as an external read-only team through the Codex -> handoff-runner -> CC -> result/log flow before the next product Work Block. Product code stayed read-only; allowed write was `memory-bank/external-team-log.md` plus ignored handoff runtime artifacts. | Claude Code external team | CC verdict: SUPPLEMENT; Codex accepted as review evidence, not automatic implementation approval | READY — runner exit 0, scope audit passed, external-team-log written; process follow-up captured for runner-owned scope-audit noise and handoff env portability |
| 2026-06-18 | wb-choushop-dirty-tree-cleanup-review | Standard review/cleanup tier | Dirty tree spans SDLC evidence, Candy Cloud frontend, commerce-flow pivot, admin/auth/schema/package changes, and local artifacts. Scope is non-destructive classification plus ignore/audit updates only; risky product/backend files stay read/classify-only. | Docs/Artifacts Reviewer; Frontend Analyst; Backend/Admin Risk Reviewer; Codex Critic | critic: RECONSIDER -> addressed by Stage 0 rerun with explicit buckets, exact cleanup whitelist, skill routing, and minimum checks | In progress — local artifact ignore cleanup applied; risky changes split into owner-decision follow-up Work Blocks |
| 2026-06-18 | wb-choushop-sdlc-navigation-sync | Standard SDLC sync tier | Sync only the framework navigation-control layer into ChouShop after pushing the safe cleanup/audit commit. Product/backend/admin/schema/package dirty files are deferred; `AGENTS.md` remains authoritative; new map/registry/bootstrap files are adapted to `memory-bank/`. | Codex Critic | critic: SUPPLEMENT — preserve ChouShop policy, adapt memory paths, add bootstrap navigation checks, and guard commit scope | In progress — navigation files synced; verification pending |
