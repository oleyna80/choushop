# Code Skeletons — Canonical Animation Patterns

Load when implementing sticky-stack, horizontal-pan, or scroll-reveal animations (Section 5 of main SKILL.md).
...
- **Layout Transitions:** Use Motion's `layout` and `layoutId` for visible state changes. Do not wrap static content in `layout` props "for safety."
- **Staggered Orchestration:** Use `staggerChildren` (Motion) or CSS cascade (`animation-delay: calc(var(--index) * 100ms)`).
